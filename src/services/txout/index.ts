...
export default class TxoutService {
  ...
  public async getTxoutByScriptHash(scripthash: string, offset: number, script?: boolean, unspent?: boolean) {
    let entity = await this.txoutModel.getTxoutByScriptHash(scripthash, offset, script, unspent);
    if (!entity) {
  ...
  public async getTxoutByAddress(address: string, offset: number, script?: boolean, unspent?: boolean) {
    let entity = await this.txoutModel.getTxoutByAddress(address, offset, script, unspent);
    if (!entity) {
      throw new ResourceNotFoundError();
    }
    return entity;
