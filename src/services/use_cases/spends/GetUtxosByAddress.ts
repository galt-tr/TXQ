...
export default class GetUtxosByAddress extends UseCase {
  ...
  public async run(params: { address: string, offset: any}): Promise<UseCaseOutcome> {
    let entities = await this.txoutService.getTxoutByAddress(params.address, params.offset, false, true);
    let utxoFormatted = [];
